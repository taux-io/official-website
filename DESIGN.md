...
條目很多的長文頁（十條以上、讀者需要知道自己讀到哪裡）可以加一條**章節索引**：`xl` 以上顯示在內文左側、`position: sticky`，隨捲動高亮當前章節；`xl` 以下**不顯示**，改由頁首的總覽表帶錨點充當目錄。

三條約束，都不是風格偏好：

- **內文閱讀寬度不變。** 索引用絕對定位掛在 `max-w-prose` 容器的左外側，不改成兩欄；`68ch` 是為中文長文定的，不能為了塞側欄而讓步。
- **索引必須放在 `space-y-*` 堆疊之外。** `space-y` 用 `> * + *` 給第一個之後的每個子元素加上邊距，而絕對定位的子元素照樣算一個元素——放進堆疊裡會把整頁往下推一個級距。
- **腳本缺席時它必須完整可用。** 見「動態」與決策 #21。

`<main>` 用 `overflow-x-clip` 而不是 `overflow-hidden`，否則 sticky 全站失效——見決策 #22。
...
| 章節索引的高亮 | **只搬移 `aria-current`，絕不寫 `opacity`。** 見決策 #21 |
| τ 曲線與生成式圖形 | 見下節 |
...
| 錨點完整性 | 片段連結必須指向渲染後那一頁真的存在的 id。**id 集合沿 `{% include %}` 遞迴組出**，因為片段是對渲染後的文件解析的，而頁首與頁尾為每一頁貢獻 id——只讀單一模板會同時漏掉真的存在的 id，也無法對寫在 partial 裡的連結說任何話。同頁片段（`#frag`）對它所在的每一頁各驗一次，所以寫在頁尾的連結必須在所有頁面上都解析得到；跨頁片段（`/path#frag`）的正確性與它寫在哪一頁無關，只驗一次 |

**規則隨著讓它們成立的那次改動各自開啟。** 這份文件先前寫著「五條裡有三條今天不成立」——那在遷移完成後就過期了，六條現在全部開著。慣例本身不變：一條還不成立的規則帶著關閉狀態進版本控制，並具名寫出負責開啟它的票號。這樣 CI 一路綠燈而不需要豁免清單——**用關掉規則換綠燈和用豁免清單換綠燈是同一件事**，差別只在前者誠實。檢查器會列出尚未開啟的規則與負責開啟它的票號。
...
| **支柱頁** | 一個主題的總覽頁：涵蓋全部條目，各條目可再拆成自己的子頁，由支柱頁內鏈過去。它承接「這個主題有哪些東西」，子頁承接「這一項是什麼」 |
| **章節索引** | 長文頁內文左側的 sticky 章節導覽，`xl` 以上才顯示。見「版面 — 長文頁」 |
...
| 21 | 章節索引的捲動高亮**不是**捲動揭示 | 決策 #13 禁的是「文字的可見性取決於腳本跑成功」。索引是十條真的錨點連結：腳本不在、載入失敗或拋錯，十條仍完整可讀、可點、可跳，失去的只有「知道自己在哪」。腳本只搬移 `aria-current`，樣式直接讀那個屬性——**一份狀態，不是兩份**。最壞的結果是某一幀標錯，不是內容消失。豁免以行為性質界定並寫在這裡，與 `#menuOverlay` 同一個理由：第二個豁免不可能在沒人開口的情況下被加進去 |
| 22 | `<main>` 用 `overflow-x-clip` 取代 `overflow-hidden` | `overflow: hidden` 會讓元素成為捲動容器，**其中的 `position: sticky` 因此全部失效**——實測捲 3000px 後 sticky 元素從 706px 移到 −2294px，完全隨頁面捲走。`clip` 不建立捲動容器，保留原本要的水平裁切（滿版 canvas 用的）而不破壞 sticky。改完在 6 條路由 × 3 種視窗下確認過沒有出現水平溢出 |
