...
- 文字落在置中的 1200px 閱讀欄內。
- **一行的字元數上限是 68ch**（`max-w-prose`），而 1200px 是欄寬不是行長——兩者不可互相代替。68 而不是拉丁常見的 75，理由是中文排得比拉丁密。**這條在內文從 17px 降到 16px 之後變得更重要而不是更不重要**：同樣的像素寬度裝進更多字，`/pqc-migration` 與 `/threat-landscape` 的導言因此從 70.8ch 變成 85.2ch（issue #147）。沒有東西檢查這一條。
...
- **68ch 的行長上限。** 讀 class 字串判斷不了一個段落最後有多寬——它取決於祖先容器、字級與字體。要量的是渲染結果，屬於 `geometry` 那一族，本次沒有寫。型階一動它就會漂移，而 issue #147 已經示範過一次。
