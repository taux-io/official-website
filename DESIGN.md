...
先前的版本把設計描述散在三個檔案裡，其中兩個已經和現況不符——`NOTES.md` 寫著「模板裡不應出現任何 hex 值」，而 `adk-skill-patterns.html` 裡有 20 個；寫著「標題 weight 400」，而模板普遍掛 `font-bold`。**規則寫了沒有東西檢查，就等於沒有規則**，所以這份文件的每一條都盡量寫成 `npm run check:design` 檢查得到的形狀。
...
自架，Latin-only，`unicode-range` 策略比照 D-DIN。授權 Apache 2.0。
...
| 檢查 | 規則 |
|---|---|
| 零 hex | `templates/**/*.html` 裡不得出現 hex 顏色值（含行內 `style` 與 `<style>` 區塊）。目前 `adk-skill-patterns.html` 有 20 個，遷移時清掉 |
| 字距 | `tracking-[...]` 的任意值只能取自刻度（`0`、`0.09em`、`0.02em`、`-0.017em`）。其餘一律報錯 |
| 圓角 | `rounded-*` 只允許出現在互動控制項（`button`、`a.btn`、`input`、`select`、`.tag`）與 `rounded-full` |
| 捲動揭示 | 不得有以 `opacity-0` 起始、由腳本解除的元素 |
| 標題結構 | 每個 `<h1>` 必須含 `.display-lead` 與 `.display-sub` 兩行 |
