...
- **全新 Docker 獨立部署腳本 (deploy.prod.sh)**:
  - 新增了專屬的生產環境部署腳本 `deploy.prod.sh`，採用方案 A (Go Gin 應用程式單一容器運作)，透過向宿主機上的 `nginx-proxy` 宣告虛擬主機與連接埠進行對接。
  - 當容器啟動時，會自動註冊對外網域 `taux.io` 與 `www.taux.io`，並藉由 `acme-companion` 與宿主機的 80/443 連接埠對外，由 `app-service@bonbondi.com` 進行 Let's Encrypt 憑證的動態發行與續簽。
