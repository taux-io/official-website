...
- `font-src 'self'` — 字體已自架
- 唯一的第三方 script 來源是邊緣注入的 Web Analytics beacon，它無法自架。`cdn.jsdelivr.net` 曾經在這裡，供一支沒有版本、沒有 `integrity` 的 Chart.js 使用；該檔案已改為自架於 `static/js/vendor/`，來源也一併移除——那個 host 供應所有 npm 套件與所有公開 GitHub repo，允許它等同於允許 `*`。
